...
class Category {
  static async createCategory(category: CategoryInterface) {
    const results = await db.query(
      `INSERT INTO categories (title, description) VALUES('${category.title}','${category.description}') RETURNING *`
    );
  ...
  static async getAllCategories() {
    ...
    const results = await db.query(
      "SELECT id, title, description, date_created FROM categories;"
    );
  ...
  static async getCategory(id: number) {
    ...
    const results = await db.query(
      `SELECT id,title, description, date_created FROM categories WHERE id = ${id}`
    );
    return results.rows[0];
  }
